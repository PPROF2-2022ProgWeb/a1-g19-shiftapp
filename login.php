<?php
  ...
  if (!empty($_POST['nombreusuario']) && !empty($_POST['contrasenia'])) {
    $records = $conn->prepare('SELECT id, nombreusuario, contrasenia from usuario WHERE nombreusuario = :nombreusuario');
    ...
    if (count($results) > 0 && password_verify($_POST['contrasenia'], $results['contrasenia'])) {
      ...
      $message = 'lo siento, la contraseña no es correcta';
    }
  }
...
<main>
  ...
  <div class="login-box">
    ...
    <h1>Ingresar</h1>
    <!-- Validacion Campos Vacios bootstrap tambien se agrego php-->
